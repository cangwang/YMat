...
float* ymat::VertexUtil::create(int width, int height, shared_ptr<PointRect> rect, float *array) {
    if (array == nullptr) {
        ELOGE("array is null");
        return array;
    }
...
//顶点区域为-1~1
float ymat::VertexUtil::switchX(float x) {
    return x * 2 - 1;
}

//顶点区域-1~1
float ymat::VertexUtil::switchY(float y) {
    return ((y*2 - 2) * -1) - 1;
}